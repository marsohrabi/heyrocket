...
import { useRockets, useRocketsPage } from '../rockets/redux/hooks';
import RocketGrid from '../components/RocketGrid';
import { RemoteData, Rocket, actions as rocketActions } from '../rockets/redux/model';
...
const RocketSearchPage: React.FC = () => {
    ...
    /* const getRocketsPage = compose(dispatch, rocketActions.fetchRocketsPage);
    const rocketsPage = getRocketsPage(2);

    if (typeof rocketsPage === "string") {
...
To save on money when shooting for far-away planets such as Jupiter, some spacecraft whip around a planet (say, Venus) and use its gravity to get a speed boost. This shortens the time it takes to get to other destinations.
                ...
                <Grid container direction="row" justify="center" className={classes.pageButtonsDiv}>
                    <Button variant="contained" className={classes.pageButton}>Previous page</Button>
                    <Button variant="contained" className={classes.pageButton}>Next page</Button>
                </Grid>
            </Grid>
        );
    };
